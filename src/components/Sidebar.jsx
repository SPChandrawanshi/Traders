...
const Sidebar = ({ onLogout, onNavigate }) => {
  const items = [
    ...
    { icon: Tag, label: 'Group Trades', onClick: () => onNavigate('group-trades') },
    { icon: Tag, label: 'Closed Trades', onClick: () => onNavigate('closed-trades') },
    { icon: Tag, label: 'Deleted Trades', onClick: () => onNavigate('deleted-trades') },
    { icon: LayoutList, label: 'Pending Orders', onClick: () => onNavigate('pending-orders') },
    { icon: Banknote, label: 'Trader Funds' },
    { icon: Users, label: 'Users' },
    { icon: FileText, label: 'Scrip Data', onClick: () => onNavigate('scrip-data') },
    ...
    { icon: Grid2X2, label: 'Bank Details For New Clients', onClick: () => onNavigate('new-client-bank') },
    { icon: BookOpen, label: 'Accounts', onClick: () => onNavigate('accounts') },
    { icon: Briefcase, label: 'Broker Accounts' },
    { icon: UserCog, label: 'Change Login Password', onClick: () => onNavigate('change-password') },
    { icon: ShieldCheck, label: 'Change Transaction Password', onClick: () => onNavigate('change-transaction-password') },
    { icon: ArrowDownRight, label: 'Withdrawal Requests' },
    { icon: ArrowUpLeft, label: 'Deposit Requests', onClick: () => onNavigate('deposit-requests') },
    { icon: AlertCircle, label: 'Negative Balance Txns', onClick: () => onNavigate('negative-balance') },
